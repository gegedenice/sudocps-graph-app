...
function datatograph(data){
    ...
    var nodes = data
                ...
                .map(function(d){return {"id":d.identity.low,"group":d.labels[0],"label":renameKey(d.properties).name}})
    console.log(nodes)
    var links = data
  ...
  function draw2D(gData){
    ...
    const Graph = ForceGraph()(elem)
        ...
        .nodeCanvasObject((node, ctx, globalScale) => {
          const label = node.label + " (" + node.group + ")";
    ...
    const Graph = ForceGraph3D({extraRenderers: [new THREE.CSS2DRenderer()]})(elem)
                     ...
                     // .nodeLabel(node => `${node.label}`)
                      ...
                      /*.nodeThreeObject(node => {
                        const sprite = new SpriteText(node.label + " (" + node.group + ")");
